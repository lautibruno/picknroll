...
describe('calcularVeredicto', () => {
  ...
  it('con MVP, es Leyenda sin importar otra cosa', () => {
    const veredicto = calcularVeredicto(carreraBase({ trofeos: { anillos: 0, allStar: 0, mvp: 1 } }))
  ...
  it('con pico de OVR 93+, es Leyenda aunque no tenga trofeos', () => {
    const veredicto = calcularVeredicto(
      carreraBase({ jugador: { ovr: 60, edad: 40, potencial: 99 }, historial: [{ edad: 30, ovr: 94, clubId: null, clubNombre: null, clubEscudoUrl: null, pj: 82, minutos: 32, rol: 'titular', ppg: 20, rpg: 5, apg: 5, triples: 1 }] }),
    )
    expect(veredicto.titulo).toBe('LEYENDA')
  })

  it('con 3+ All-Star, es Estrella NBA', () => {
    const veredicto = calcularVeredicto(carreraBase({ trofeos: { anillos: 0, allStar: 3, mvp: 0 } }))
    expect(veredicto.titulo).toBe('ESTRELLA NBA')
  })

  it('con al menos un All-Star, es Buen Titular', () => {
    const veredicto = calcularVeredicto(carreraBase({ trofeos: { anillos: 0, allStar: 1, mvp: 0 } }))
  ...
  it('usa el pico histórico de OVR, no el final (que puede estar en declive)', () => {
    const veredicto = calcularVeredicto(
      carreraBase({
        ...
        historial: [{ edad: 29, ovr: 90, clubId: null, clubNombre: null, clubEscudoUrl: null, pj: 82, minutos: 32, rol: 'titular', ppg: 25, rpg: 6, apg: 5, triples: 1 }],
      }),
    )
    expect(veredicto.titulo).toBe('ESTRELLA NBA')
  })
