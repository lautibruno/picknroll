...
export function PantallaEvento({ carrera, onElegir }: PantallaEventoProps) {
  ...
  if (evento.tipo === 'riesgo') {
    ...
    return (
      <div className="mx-auto flex h-dvh max-w-2xl flex-col border-2 border-hueso/15 bg-fondo sm:h-auto">
        <Hud carrera={carrera} />
        ...
        {evento.opciones.map((opcion: Equipo) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col gap-1.5 border-t-4 bg-fondo px-2 py-2.5 text-left transition-opacity hover:bg-superficie sm:gap-3 sm:px-5 sm:py-6 ${
                ...
                >
                  <div className="flex flex-col items-center gap-1.5 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center border-2 border-hueso/25 bg-superficie-alta font-titulo text-[9px] font-semibold sm:order-2 sm:h-11 sm:w-13 sm:text-sm">
                      {opcion.escudoUrl ? (
                        <img src={opcion.escudoUrl} alt="" className="h-7 w-7 object-contain sm:h-9 sm:w-9" />
                      ) : (
                        abreviarNombre(opcion.nombre)
                      )}
                    </div>
                    <div className="line-clamp-2 text-center font-titulo text-[11px] font-semibold uppercase leading-tight tracking-[0.02em] sm:order-1 sm:text-left sm:text-xl sm:tracking-[0.04em]">
                      {opcion.nombre}
                    </div>
                  </div>
                  ...
                  <div className="hidden gap-0.5 bg-hueso/10 sm:flex">
                  ...
                  <div className="mt-auto flex items-center justify-center border-t border-hueso/15 pt-1.5 sm:justify-between sm:pt-3">
                    <span className={`hidden font-mono-stats text-[9px] tracking-[0.1em] sm:inline ${esQuedarse ? 'text-hueso/50' : 'text-acento'}`}>
                      {esQuedarse ? 'QUEDARTE ACÁ' : etiquetaRiesgoClub(opcion.nivel, carrera.jugador.ovr)}
                    </span>
                    <span className="w-full bg-hueso px-2 py-1.5 text-center font-titulo text-[10px] font-semibold tracking-[0.1em] text-fondo sm:w-auto sm:px-4 sm:py-2 sm:text-xs sm:tracking-[0.16em]">
